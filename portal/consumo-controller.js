...
// ******************************************** GET DATA ********************************************************************
 ...
 // ============================================== Calcular Pegada Hídrica Pessoal ==============================
    ...
    function calcWaterFootprint(element,value){
        ...
        $j('.resultadoCategoria#totalHigiene').show().find('span').text(totalHigiene);
        $j('.resultadoCategoria#totalLimpeza').show().find('span').text(totalLimpeza);
        $j('.resultadoCategoria#totalAlimento').show().find('span').text(alimentoTotalZeradoNum);
        $j('.resultadoTotal').find('span').text(totalPegadaPessoal);
...
$scope.changeWaterFootprint = function(key,value){
    ...
    $j('.resposta').keypress(function(event) {
      if(event.which < 44
      || event.which > 59) {
    ...
    $scope.nextSlide = function() {
      ...
      var slider = $j('.minhaPegada .slider');
      var actualSlide;
      var actualSlideHeight;
      if (slider.hasClass('slide-0')){
        actualSlideHeight = $j('.minhaPegada .slidebox1').find('.pageWrapper').outerHeight()+115;
        slider.height(actualSlideHeight);
      } else if (slider.hasClass('slide-1')){
        actualSlideHeight = $j('.minhaPegada .slidebox2').find('.pageWrapper').outerHeight()+115;
        slider.height(actualSlideHeight);
      } else {
      }
    };
    ...
    $scope.prevSlide = function() {
      ...
      if (slider.hasClass('slide-0')){
        ...
        actualSlideHeight = $j('.minhaPegada .slidebox0').find('.pageWrapper').outerHeight()+115;
        slider.height(actualSlideHeight);
      } else {
          actualSlideHeight = $j('.minhaPegada .slidebox1').find('.pageWrapper').outerHeight()+115;
        slider.height(actualSlideHeight);
    ...
    $scope.submitAll = function() {
      ...
      $j('.resultadoTotalWrapper').show(150).delay(500).queue(function() {
           $j('.goToHome').show().addClass('animated zoomIn');
           $j(this).dequeue();
        });
